...
{
    // size_t *virt_pgd_parent = (size_t*)PHYS_TO_VIRT((char*)pgd_parent);
    // size_t *virt_pgd_child = (size_t*)PHYS_TO_VIRT((char*)pgd_child);
    int i = 511;
    if (virt_pgd_parent[i] != 0){}
    for (int i = 0; i < 512; i++)
    {
        if (virt_pgd_parent[i] != 0)
        {
            ...
            if (level == 0)
            {
                virt_pgd_child[i] = virt_pgd_parent[i];
            }

            if (level == 3){
                virt_pgd_parent[i] |= PD_RDONLY;
                continue;
            }
            size_t *next_table_parent = (size_t*)PHYS_TO_VIRT((size_t)(virt_pgd_parent[i] & ENTRY_ADDR_MASK));
            if (level != 3) copy_PTE(next_table_parent, next_table_parent, level + 1);
        }
    }
}
